...
<head>
  ...
  <style>
    ...
    /* Modified colors for distinct appearance */
    .student-by-number {
      color: #4caf50; /* Green */
    }
    .students-by-score {
  ...
  <div class="container">
    ...
    <?php
      ...
      function displayStudentByNumber($number, $data) {
        foreach ($data as $student) {
          if ($student['No Urut'] == $number) {
            echo "<p class='student-by-number'>Nama siswa dengan nomor urut $number adalah <strong>" . $student['Siswa'] . "</strong> dengan poin <strong>" . $student['Poin'] . "</strong></p>";
            break;
          }
        }
      }
      ...
      function displayStudentsByScore($score, $data) {
        ...
        if (count($students) > 0) {
          echo "<p class='students-by-score'>Nama siswa yang memiliki poin $score adalah <strong>" . implode(', ', $students) . "</strong></p>";
        } else {
          echo "<p class='students-by-score'>Tidak ada siswa yang memiliki poin $score</p>";
        }
      }
